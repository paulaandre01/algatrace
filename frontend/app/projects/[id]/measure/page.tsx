    ...
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-6 text-center px-4">
        <div className="bg-slate-100 p-6 rounded-full">
          <Leaf className="h-16 w-16 text-slate-400" />
        </div>
        <div className="max-w-md space-y-2">
          <h2 className="text-2xl font-bold text-slate-900">Conecte sua carteira</h2>
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto animate-in fade-in duration-500">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Nova Medição MRV</h1>
          <p className="text-slate-500 mt-1">Registre o crescimento de biomassa para cálculo de CO₂.</p>
        </div>
      </div>

      <Card className="p-8 shadow-lg border-slate-200/60">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="p-5 bg-blue-50/50 border border-blue-100 rounded-xl text-sm text-blue-900 flex items-start">
            <Activity className="h-5 w-5 mr-3 text-blue-600 flex-shrink-0 mt-0.5" />
            <div>
              <h4 className="font-bold mb-1">Metodologia Beta v0.1</h4>
              <p className="text-blue-800 leading-relaxed">
                O cálculo utiliza o fator de conversão padrão de <strong className="font-mono bg-blue-100 px-1 rounded">{MRV_CONSTANTS.CO2_CONVERSION_FACTOR} kg CO₂</strong> por kg de biomassa seca de algas.
              </p>
            </div>
          </div>

          <div className="space-y-3">
            <label htmlFor="biomass" className="text-sm font-semibold text-slate-700 block">
            ...
            <div className="relative">
              <input
                ...
                step="1"
                required
                className="block w-full rounded-xl border-slate-200 pl-4 pr-12 py-3 text-lg focus:border-green-500 focus:ring-green-500 shadow-sm transition-all"
                placeholder="Ex: 100"
                value={biomass}
          ...
          <div className="pt-2">
            <Button type="submit" className="w-full h-12 text-base shadow-lg shadow-green-900/10" disabled={loading || !biomass}>
              {loading ? (
                <>
